...
```{r ratio-detect, eval = TRUE, fig.width=8, fig.height=10}
# Compute median BPC/TIC ratio per RT bin
compute_bin_ratio <- function(rt, int_tic, int_bpc, n_bins = 20) {
...
ratio_all <- sapply(seq_along(tic_detect), function(i) {
  ...
  int_bpc <- intensity(bpc_detected[i])[[1]]
...
ylim_coverage <- c(0, max(coverage_all, na.rm = TRUE))
xlim_coverage <- c(1, n_bins + 1)  # extra space at the end
# Setup
# 
ylim_ratio <- range(ratio_all, na.rm = TRUE)
xlim_ratio <- c(1, n_bins + 1)  # same extra space

n_bins <- nrow(coverage_all)
cols <- rainbow(n_samples)  # colors for the lines

for (mix in mixtures) {
  par(mfrow = c(2, 1), mar = c(3, 3, 2, 0))
  cols <- grep(mix, colnames(ratio_all))
  tmp_ratio <- ratio_all[, cols]
  tmp_coverage <- coverage_all[, cols]
  
  plot(tmp_coverage[, 1], type = "b", pch = 16, col = cols[1],
       ylab = "Median BPC/TIC ratio", xlab = "", xaxt = "n",
       ylim = ylim_coverage, xlim = xlim_coverage,
       main = paste("Median BPC/TIC ratio per RT bin - Mixture", mix))
  for (i in 2:ncol(tmp_coverage)) {
    lines(tmp_coverage[, i], type = "b", pch = 16, col = cols[i])
  }
  grid()
  
  plot(tmp_ratio[, 1], type = "b", pch = 16, col = cols[1],
       ylab = "Number of data points", xlab = "", xaxt = "n",
       ylim = ylim_ratio, xlim = xlim_ratio,
       main = paste("Number of data points per RT bin - Mixture", mix))
  for (i in 2:ncol(tmp_ratio)) {
    lines(tmp_ratio[, i], type = "b", pch = 16, col = cols[i])
  }
  axis(1, at = 1:n_bins, labels = rownames(tmp_ratio), las = 2, cex.axis = 0.8)
  mtext("RT bin", side = 1, line = 4)
  grid()
  legend("top", legend_obj)
    
}


col_info <- strsplit(colnames(coverage_all), "_")
mixture_names <- sapply(col_info, `[`, 2)
lab_names <- sapply(col_info, `[`, 1)
unique_labs <- unique(lab_names)
n_labs <- length(unique_labs)

# Assign colors per lab
cols_lab <- rainbow(n_labs)
cols <- cols_lab[match(lab_names, unique_labs)] 
...
# Assign names
colnames(ratio_all) <- paste(tic_detect$lab, 
                             tic_detect$mixture, "pol:", tic_detect$polarity, sep = "_")
colnames(coverage_all) <- colnames(ratio_all)
...
for (mix in mixtures) {
  ...
  axis(1, at = 1:n_bins, labels = rownames(tmp_ratio), 
       las = 2, cex.axis = 0.8)
  mtext("RT bin", side = 1, line = 4)
  grid()
  legend("top", legend_obj)
...
sim_matrix <- matrix(NA, ncol = length(full_int), 
                     nrow = length(full_int))
...
# heatmap 
heatmap(sim_matrix, labRow = paste(tic_ann$lab, 
                                   tic_ann$mixture, sep = "_"),
...
for (i in seq_along(full_int)) {
  for (j in seq_along(full_int)) {
    if (is.na(sim_matrix[i, j])) {
      dtw_res <- dtw(full_int[[i]], full_int[[j]],
                     keep = FALSE)
      sim_matrix[i, j] <- dtw_res$normalizedDistance
      sim_matrix[j, i] <- sim_matrix[i, j]
    }
...
# cluster 
hc <- hclust(as.dist(sim_matrix))
plot(hc, labels = paste(bpc_ann$lab, bpc_ann$mixture, 
                        sep = "_"), 
     main = "DTW clustering of TIC")

# heatmap 
heatmap(sim_matrix, labRow = paste(bpc_ann$lab,
                                   bpc_ann$mixture,
                                   sep = "_"),
...
```{r}
sim_matrix_med <- matrix(NA, ncol = ncol(ratio_all), 
                         nrow = ncol(ratio_all))
for (i in seq_len(ncol(ratio_all))) {
  for (j in seq_len(ncol(ratio_all))) {
    if (is.na(sim_matrix_med[i, j])) {
      dtw_res <- dtw(ratio_all[, i], ratio_all[, j], 
                     keep = FALSE)
      sim_matrix_med[i, j] <- dtw_res$normalizedDistance
      sim_matrix_med[j, i] <- sim_matrix_med[i, j]
    }
...
# heatmap
heatmap(sim_matrix_med, labRow = colnames(ratio_all),
  labCol = colnames(ratio_all), 
  main = "DTW similarity of BPC/TIC ratio")
```
...
This describe some "other signal": together adduct,
fragmentation, ...
