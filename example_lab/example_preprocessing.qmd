...
sIn case one or more of the above libraries are not available (and hence fail
to load), they can be installed with the R command `BiocManager::install()`
(e.g. `BiocManager::install("alabaster.base")` to install the *alabaster.base*
R package). Note that this requires the installation of the *BiocManager* R
...
```{r ppm-est}
mz <- mz(eicsMH["CHEBI:15611-[M+Na]+"]) |> as.vector()
cst <- x_mse |>
    spectra() |>
    filterMsLevel(msLevel = 1L) |>
    filterRt(rt = c(24, 27)) |>
    filterMzRange(mz = c(mz[1] + 0.01, mz[2] - 0.01))
...
if (length(unique(sampleData(mse)$ms_level)) > 1) {
    ...
    saveMsObject(mse2,
             AlabasterParam(path = file.path("results/HE/mse2")))
    saveMsObject(mse,
