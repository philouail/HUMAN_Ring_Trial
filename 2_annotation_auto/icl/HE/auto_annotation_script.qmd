---
title: "automatic annotation icl, HE."
format: html
editor: source
---
...
res_path <- file.path("1_preprocessing", lab, study_group)
out_path <- file.path("2_annotation_auto", lab, study_group)
...
mse <- readMsObject(XcmsExperiment(),
            AlabasterParam(path = file.path(res_path, "mse")),
            spectraPath = file.path(res_path, "mzml"))
sampleData(mse)$mixture <- sub(".*_", "", sampleData(mse)$Sample.Name)
sampleData(mse)$mixture <- sub("\\.mzML", "", sampleData(mse)$mixture)
...
for (i in seq_along(list_res$spectra)) {
    ...
    dr_tmp <- file.path(out_path, list_res$data[a$chrom_peak_id, "mixture"], "ms1_adduct_plots")
...
```{r}
for (i in seq_along(list_res$spectra)) {
    dr_tmp <- file.path(out_path, list_res$data$mixture[i], "ms1_isotopes_plots")
    dir.create(dr_tmp, showWarnings = FALSE, recursive = TRUE)
...
If you have separated MS2 spectra in the pre-processing step, you load it below

```{r}
mse2 <- readMsObject(
  XcmsExperiment(),
  AlabasterParam(path = file.path(res_path, "mse2")),
  spectraPath = file.path(res_path, "mzml")
)
sampleData(mse2)$mixture <- sub(".*_", "", sampleData(mse)$Sample.Name)
sampleData(mse2)$mixture <- sub("\\.mzML", "", sampleData(mse)$mixture)
sampleData(mse2)$mixture <- gsub("\\.", "_", sampleData(mse2)$mixture)

ms2peaks <- automate_matching_ms2(
  mse2,
  list_res$data,
  toleranceMz = 0.1,
  toleranceRt = 5,
...
# ref DB
dbfile <- file.path("2_annotation_auto",
...
for (i in seq_len(nrow(toplot))) {
  dr_tmp <- file.path(out_path, toplot$mixture[i], "ms2_plots")
  dir.create(dr_tmp, showWarnings = FALSE, recursive = TRUE)
...
```{r}
naps_res <- read.csv(file.path(res_path, "naps.csv"))
...
write.csv(match_res,
          file = file.path(out_path, "peak_evidence.csv"), row.names = FALSE)
```
...
```{r}
write.csv(res_all,
          file = paste0(outpath, "/peak_evidence_rt_grouped.csv"),
          row.names = FALSE)
```
...
for (std in unique(res_all$target_ChEBI.name)) {
  ...
  for (i in seq_len(nrow(subset_std))) {
    ...
    for (cpd in cpds) {
      dr_tmp <- file.path(out_path, subset_std$mixture[i], "grouped_eic_plots")
      dir.create(dr_tmp, showWarnings = FALSE, recursive = TRUE)
