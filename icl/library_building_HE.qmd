...
library(pander)
tmp <- data.frame(
    count = c(standards = nrow(meta),
              inchikey = length(unique(meta$InChIKey)),
              chrom_peaks = nrow(match_res),
              matched_inchikey = length(unique(match_res$target_InChIKey))))
pandoc.table(tmp, style = "rmarkdown")
```

The numbers of matched chrom peaks per unique InChIKey:

```{r}
quantile(table(match_res$target_InChIKey))
```

The most commonly matched adducts:

```{r}
match_res$adduct |> table() |> sort()
```
...
Information on the number of MS2 per chrom peaks are shown below.

```{r, results = "asis"}
tmp_idx <- !is.na(list_res$data$ms2_count)
tmp <- data.frame(
    count = c(chrom_peaks = nrow(list_res$data),
              chrom_peaks_with_ms2 = sum(tmp_idx),
              inchikey_with_ms2 = length(
                  unique(list_res$data[tmp_idx, "target_InChIKey"]))))
pandoc.table(tmp, style = "rmarkdown")
```

Below we check for which adducts we have MS2 spectra.

```{r}
table(list_res$data[tmp_idx, "adduct"]) |> sort()
```
...
## funfun <- function(x, precursorMz, original_precursorMz, ...) {
##     d <- precursorMz - original_precursorMz
##     x[, 1L] <- x[, 1L] - d
##     x
## }

## ms2peaks <- addProcessing(
##     ms2peaks, funfun,
##     spectraVariables = c("precursorMz", "original_precursorMz"))
...
## Evaluate which spectra are matching, which not.
...
```{r, results = "asis"}
tmp <- list_res$data[!is.na(list_res$data$ms2_count), ]

tmp_df <- data.frame(
    count = c(chrom_peaks_with_ms2 = nrow(tmp),
              inchikey_with_ms2 = length(unique(tmp[, "target_InChIKey"])),
              matched_ms2 = sum(!is.na(tmp$ms2_matched_count)),
              true_matched_ms2 = sum(!is.na(tmp$ms2_true_count))
              ))
pandoc.table(tmp_df, style = "rmarkdown")
```

```{r, eval = FALSE}
## Original setting
csp <- CompareSpectraParam(ppm = 40, tolerance = 0.05, requirePrecursor = TRUE,
                           THRESHFUN = function(x) which(x >= 0.6))
a <- matchSpectra(ms2peaks, ms2_ref, param = csp)

## csp <- CompareSpectraParam(ppm = 40, tolerance = 0.05, requirePrecursor = FALSE,
##                            THRESHFUN = function(x) which(x >= 0.6))
## b <- matchSpectra(ms2peaks, ms2_ref, param = csp)

csp <- CompareSpectraParam(ppm = 40, tolerance = 0.1, requirePrecursor = TRUE,
                           THRESHFUN = function(x) which(x >= 0.6))
b <- matchSpectra(ms2peaks, ms2_ref, param = csp)
