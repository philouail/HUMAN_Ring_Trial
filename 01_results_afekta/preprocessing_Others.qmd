...
study_group <- "Others" #example HE / FE / Others
```
...
```{r}
cores_nb <- 10 # Adjust the number of cores based on your machine
...
plot(eicsMH["CHEBI:17780-[M+H]+"], xlim = c(165, 175)) # how large is the peak ? 7 sec from 167 to 174 sec
plot(eicsMH["CHEBI:18019-[M+H]+"], xlim = c(24, 32)) # how large is the peak ? 6 sec from 24 to 30 sec
plot(eicsMH["CHEBI:15611-[M+H]+"], xlim = c(24, 30)) # how large is the peak ? 6 sec from 24 to 30
```
...
```{r}
mz <- mz(eicsMH["CHEBI:17780-[M+H]+"]) |> as.vector() ## choose compound here

x_mse |>
    filterMsLevel(1L) |>
    filterRt(c(165, 175)) |>
    filterMzRange(mz = c(221.084, 221.09)) |> # play around with this to determinate the best range
    plot()

cst <- x_mse |>
    spectra() |>
    filterMsLevel(msLevel = 1L) |>
    filterRt(rt = c(165, 175)) |> # be restrictive 
    filterMzRange(mz =  c(221.084, 221.09)) # set it up as above.
...
```{r}
ppmval <- 20 ## input your number here #Min 20ppm?
```
