...
automate_matching <- function(meta, mse) {
    ...
    for (x in mixtures) {
        ...
        # Add adduct m/z
        match_res$adduct_mz <- mapply(match_res$target_M, match_res$adduct, FUN = mass2mz)

        # Add adduct formula
        af <- mapply(match_res$target_formula, match_res$adduct, FUN = adductFormula)
...
calculate_isotope_similarity <- function(ms1_spectra, match_df) {
    ...
    list(data = match_df, spectra = iso_spectra, theoretical = theoretical_spectra)
}
...
match_ms2_to_library <- function(ms2_spectra, ref_spectra, meta_df, min_score = 0.6) {
    ...
    df <- matchedData(mtch)[, c("chrom_peak_id", "target_compound_name",
...
update_all_with_rti <- function(match_res, mse, naps_res, polarities = c("pos", "neg")) {
