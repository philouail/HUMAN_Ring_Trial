...
setClass(
  "SavitzkyGolayParam",
  slots = c(window = "integer", polynomial = "integer"),
  prototype = list(window = 11L, polynomial = 2L),
...
SavitzkyGolayParam <- function(window = 11, polynomial = 2) {
  new(
    ...
    polynomial = as.integer(polynomial)
  )
}
...
setMethod(
  "processEICs",
  signature(object = "Chromatograms", param = "SavitzkyGolayParam"),
  function(object, param) {
    ...
    res <- lapply(seq_along(df_list), function(i) {
      ...
      if (nrow(df) >= param@window) {
        ...
        warning(
          sprintf(
            "EIC %d: Skipped smoothing (n=%d < window=%d)",
            i,
            nrow(df),
            param@window
          ),
...
setMethod(
  "processEICs",
  signature(object = "Chromatograms", param = "IsolatePeakParam"),
  function(object, param) {
    df_list <- peaksData(object)
    ...
    res <- lapply(df_list, function(df) {
      ...
      if (is.null(peaks)) {
        warning(
          "No peaks found in chromatograms; returning empty data frame.",
          call. = FALSE
        )
      ...
      while (
        left > 1 &&
          df$intensity[left - 1] < df$intensity[left] &&
          df$intensity[left - 1] > threshold
      ) {
        left <- left - 1
      }
      right <- main_center
      while (
        right < n &&
          df$intensity[right + 1] < df$intensity[right] &&
          df$intensity[right + 1] > threshold
      ) {
        right <- right + 1
      }
...
flagBadEICs <- function(
  ...
  if (!requireNamespace("pracma", quietly = TRUE)) {
    stop(
      "Package 'pracma' is required. Please install it with install.packages('pracma')"
    )
  }
...
setClass(
  "BaselineParam",
  slots = c(lambda = "numeric", p = "numeric"),
  prototype = list(lambda = 1e5, p = 0.01),
  validity = function(object) {
    if (object@lambda <= 0) {
...
setMethod(
  "processEICs",
  signature(object = "Chromatograms", param = "BaselineParam"),
  function(object, param) {
    # Ensure the required package is installed
    if (!requireNamespace("ptw", quietly = TRUE)) {
      stop(
        "Package 'ptw' is required for baseline correction. Please install it."
      )
    }
    ...
    res <- lapply(df_list, function(df) {
      ...
      if (nrow(df) > 0 && sum(df$intensity, na.rm = TRUE) > 0) {
        ...
        corrected_intensity <- ptw::baseline.corr(
          df$intensity,
          lambda = param@lambda,
          p = param@p
        )
...
calculatePeakMetrics <- function(df_list) {
  ...
  metrics_list <- lapply(seq_along(df_list), function(i) {
    ...
    fwhm <- tryCatch(
      {
        ...
        if (
          min(rising_side$intensity, na.rm = TRUE) <= half_max &&
            min(falling_side$intensity, na.rm = TRUE) <= half_max
        ) {
          rt_left <- approx(
            rising_side$intensity,
            rising_side$rtime,
            xout = half_max
          )$y
          rt_right <- approx(
            falling_side$intensity,
            falling_side$rtime,
            xout = half_max
          )$y
          rt_right - rt_left
    ...
    tailing_factor <- tryCatch(
      {
        ...
        if (
          min(rising_side$intensity, na.rm = TRUE) <= height_10pct &&
    ...
    gaussian_similarity <- tryCatch(
      {
        ...
        if (is.na(fwhm) || fwhm == 0) {
          ...
          y_ideal <- apex_intensity *
            exp(-((df$rtime - rt_apex)^2) / (2 * sigma^2))
    ...
    bs_apex_ratio <- ifelse(
      apex_intensity > 0,
      baseline / apex_intensity,
      NA_real_
    )
...
plot_signal_comparison <- function(
  signal_to_plot = "TIC",
  polarity_to_plot = 1
) {
  # --- A. Load Data Objects ---
  ...
  tryCatch(
    ...
    error = function(e) {
      stop(
        "Could not load data files. Make sure objects like 'tic_full.RData' or 'bpc_full.RData' exist in the 'object/' folder."
      )
    }
  )
  ...
  comparison_plot <- ggplot(
    ...
    labs(
      title = paste(
        signal_to_plot,
        "Area Comparison by Lab and Signal Type (",
        pol_label,
        "Polarity)"
      ),
