...
mnpp <- MergeNeighboringPeaksParam(expandRt = 6, expandMz = 0.0015,
                                   minProp = 0.75)
mse_pos <- refineChromPeaks(mse_pos, mdpp, chunkSize = 2L)
print("Positive mode:")
chromPeakData(mse_pos)$merged |>
                         table() 

mse_neg <- refineChromPeaks(mse_neg, mnpp, chunkSize = 2L)
print("Negative mode:")
...
We have thus only reference spectra for a relatively low number of
compounds. What's even more surprising is that, although we filtered above for
positive polarity spectra, negatively charged adduct definitions are reported:
...
m/z similarity settings accounting for potential uncalibrated or low resolution spectra.
...
df$query_inchikey <- x_match_res[df$chrom_peak_id, "target_InChIKey"]
...
x_match_res[names(tmp), "ms2_true_count"] <- as.integer(tmp)
```
...
For each standard:
- group identified/assigned chrom peaks into *RT regions*. This should put
  adduct chrom peaks into the same group.
...
##  score : normalized dot product spectra similarity score
