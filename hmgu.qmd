...
- should we use NAPS for alignment between POS and NEG samples ? Or just the
  known standard peaks ? 
...
I've decided to go with 500 for now and accepting we will be loosing some
peaks.
...
-   for one standard: group chromatographic peaks (between the different
...
then each standard as a number of rows matching the number of regions.
...
- Another output should be the mirror polot with name of compound at the top. 
...
```{r}
## plot 
## somehow use function targetIndex and query index
...
```{r}
cpd <- "5-hydroxy-L-tryptophan"
idx <- which(cpd == x_match_res$target_ChEBI.name)
...
## all rt per compound 
library(MsFeatures)
rts  <- chromPeaks(x_mse_pos)[x_match_res$chrom_peak_id[idx], "rt"]
param <- SimilarRtimeParam(diffRt = 10, groupFun = groupClosest)
groups <- groupFeatures(rts, param)

## next i need to see what i would to do with this
## i would like to summaries and have one row per groups. 
```
