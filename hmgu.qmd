...
-   Similarity between LC runs (i.e. BPC or TIC similarities) might not be that
    ...
    -   I mean we expect the NAPS to be similar but what if they are not ? and
        difference in NAPS between labs is also interesting.
...
Below we extract EICs using theoretical m/z for both main positive adducts and
all along the retention time.
...
lab.First quickly checking a few peaks to look for peak width:
...
digraph sample_analysis {
  ...
  MatchStandards  [label = '4. Match chromatographic peaks to Standards (Theoretical m/z)']
...
```{r}
dr <- file.path("res", "hmgu") ## adapt this
