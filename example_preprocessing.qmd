...
## with the mzml files. and will be your results folder in the "results" folder.
lab <- "example"
```

```{r load-std-meta, warning=FALSE, message=FALSE}
#' Load compound metadata (sheet 2);
meta <- read_xlsx(paste0("data/standards.xlsx"), 
...
# Load pos data
seq_pos <- read_xlsx(paste0("data/", lab, "/HE_mzml/seq_pos.xlsx"), 
                 col_names = TRUE) |> as.data.frame()
seq_pos$polarity <- "pos"
# load neg data
seq_neg <- read_xlsx(paste0("data/", lab, "/HE_mzml/seq_neg.xlsx"), 
                 col_names = TRUE) |> as.data.frame()
...
write.csv(
    ...
    file = file.path("results/", lab, "HE_naps.csv"),
    row.names = TRUE
)
```
...
saveMsObject(mse,
             AlabasterParam(path = file.path("results/",
                                             lab, "/objects/mse")))
```
