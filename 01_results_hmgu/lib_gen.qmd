...
library(readxl)
res_table <- read_xlsx(file.path(dr, "peak_evidence_rt_grouped_manual_Pualine.xlsx"))
```
...
res_table <- res_table[!is.na(res_table$`T/F`), ]
res_true <- res_table[res_table$`T/F` == "TRUE", ] 
...
```{r}
test <- chromPeakSpectra(mse, msLevel = 2L, peaks = cpds, expandRt = 5,
  expandMz = 0.005)
```
