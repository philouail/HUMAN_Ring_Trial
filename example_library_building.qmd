...
The actually script you will use in the file named: generic_library_building.Rmd
...
```{r}
meta <- read_xlsx(paste0("data/standards.xlsx"), sheet = 2L, 
                      col_names = TRUE, skip = 1, .name_repair = "minimal") |>
    as.data.frame(check.names = FALSE)

## we subset for mixture 1 for this example
meta <-  split(meta, meta$Mixture)[[1]]
...
mse <- readMsObject(XcmsExperiment(), 
                  AlabasterParam(path = file.path("results/example/objects/mse")), 
                  spectraPath = file.path("data/example/HE_mzml/"))
...
dbfile <- "data/MsBackendSql.GNPS.matchms.cleaned.v1.sqlite"
