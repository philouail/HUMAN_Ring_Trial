...
When performing this automatically the object will be split so we can evaluate
mixture by mixture. To describe the process in this file we will show do it for
one mixture.
...
adds <- c("[M+H]+", "[M+Na]+") ## easily scalable/adaptable 
match_pos <- matchValues(cpks, x_meta,
                       Mass2MzParam(adds, tolerance = 0.05, ppm = 10), 
                       mzColname = "mz", massColname = "M")
match_pos <- match_pos[whichQuery(match_pos)]

adds <- c("[M-H]-", "[M+CHO2]-" ) ## the second one is "[M+FA-H]-", 
##i have duplication here.. and it screws up the MS1 extraction :/ for now i reduced the tolerance
## we need to discuss what to do in these cases bc it will happen again even with low tolerance i guess.. 
cpks <- as.data.frame(chromPeaks(x_mse_neg))
cpks$chrom_peak_id <- rownames(cpks)
match_neg <- matchValues(cpks, x_meta,
                            Mass2MzParam(adds, tolerance = 0, ppm = 10), 
                            mzColname = "mz", massColname = "M")
match_neg <- match_neg[whichQuery(match_neg)]

# so, one idea to deal with peaks that match multiple compound would be to only
...
match_res_pos <- matchedData(
    match_pos, c("chrom_peak_id","mz", "rt", "rtmin", "rtmax", "into", "target_ChEBI name",
               "target_ChEBI", "target_InChIKey", "target_formula", "target_M",
               "adduct", "score", "ppm_error", "target_theo_logP")) |> as.data.frame()

match_res_pos$polarity <- "pos"

match_res_neg <- matchedData(
    match_neg, c("chrom_peak_id","mz", "rt", "rtmin", "rtmax", "into", "target_ChEBI name",
               "target_ChEBI", "target_InChIKey", "target_formula", "target_M",
               "adduct", "score", "ppm_error", "target_theo_logP")) |> as.data.frame()
...
match_ms1 <- chromPeakSpectra(
    ...
    method = "closest_rt")  ## this function fails if duplicated peaks. i need to do a example.
match_ms1$exactmass <- match_res$target_M
match_ms1$adduct_mz <- match_res$adduct_mz

## the polarity should be there no ? YES 1 - pos and 0 - neg
spectraData(match_ms1)
...
We plot these to file. An example plot is shown below. - will deal with
plotting later.
...
#' Plot all
dr <- file.path("res", "example", "ms1_adducts_plot")
...
should it be done by regions ? would make sense. nope b ecause polarity

```{r}
naps_info <- read.csv("res/hmgu/HE_naps.csv")
## i think using mclosest makes the most sense here. 
## pos
x <- match_res[match_res$polarity == "pos", c("rt", "mz")]
table <- naps_info[, c("rt_pos", "exp_mz_pos")]
naps_closest <- mclosest(x, table)
match_res[ match_res$polarity == "pos","NAPS_closest"] <- naps_info$Name[naps_closest]
match_res[ match_res$polarity == "pos","NAPS_closest_logp"] <- naps_info$logP[naps_closest]

## neg
x <- match_res[match_res$polarity == "neg", c("rt", "mz")]
table <- naps_info[, c("rt_neg", "exp_mz_neg")]
naps_closest <- mclosest(x, table)
match_res[ match_res$polarity == "neg","NAPS_closest"] <- naps_info$Name[naps_closest]
match_res[ match_res$polarity == "neg","NAPS_closest_logp"] <- naps_info$logP[naps_closest]
```
...
split(x, x$rt_region) |>
    lapply(function(z) {
        ...
        if (length(unique(z$NAPS_closest)) > 1) {
            res$NAPS_closest <- paste(unique(z$NAPS_closest), collapse = "|")
            res$NAPS_closest_logp <- paste(unique(z$NAPS_closest_logp), collapse = "|")
        } else {
            res$NAPS_closest <- unique(z$NAPS_closest)
            res$NAPS_closest_logp <- unique(z$NAPS_closest_logp)
        }
        res
    }) |>
    do.call(what = rbind) |>
