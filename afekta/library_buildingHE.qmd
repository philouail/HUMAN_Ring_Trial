...
sampleData(mse)$mixture <- sub(".*_", "", sampleData(mse)$files)
...
mse2 <- readMsObject(XcmsExperiment(), 
            ...
            spectraPath = file.path(paste0( study_group, "_mzml/")))
sampleData(mse2)$mixture <- sub(".*_", "", sampleData(mse)$files)
...
```{r}
res_all <- read.csv(file.path(dr, "peak_evidence_rt_grouped_he_Dennisse.csv"))
```


```{r}
res_all <- res_all[!is.na(res_all$T.F.NA), ]

res_all <- res_all[res_all$T.F.NA == "TRUE", ]
